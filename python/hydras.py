...
class Hydra(Core):
    ...
    def __init__(self, source='', start='', finish='', extensions=None, show=True):
        ...
        self.extensions = extensions or ('.nc', '.he4', '.h5', '.nc4', '.he5', '.hdf')
    ...
    def _correlate(self, abscissa, ordinate, masking=True):
    ...
    def _orient(self, degrees, east=False, clip=False):
        ...
        # get digits length
        length = len(str(int(abs(degrees))))
        ...
        tag = self._pad(int(abs(degrees)), pad) + 'N' * (degrees >= 0) + 'S' * (degrees < 0)
        ...
        if east:
            ...
            # construct tag
            tag = self._pad(int(abs(degrees)), pad) + 'E' * (degrees >= 0) + 'W' * (degrees < 0)
    ...
    def ingest(self, path=0, mode=None, discard=True, names=None, folder='tmp', scan=False, net=False):
        ...
        if self._stage(path)['extension'] in ('.he4', '.he', '.hdf'):
    ...
    def plant(self, data, target, destination, masking=True, header=None, classify=False, decompose=False, give=False):
